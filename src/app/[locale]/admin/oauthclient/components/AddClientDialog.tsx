...
const AddClientDialog = ({ show, setShow, client }: Props): ReactNode => {
    ...
    const validateForm = () => {
        ...
        if (!formState.authorities.trim()) {
            newErrors.authorities = 'Authorities is required'
        }
        ...
        if (!formState.accessTokenValidity) {
            newErrors.accessTokenValidity = 'Access Token Validity is required'
        }
        if (!formState.refreshTokenValidity) {
            newErrors.refreshTokenValidity = 'Refresh Token Validity is required'
        }
    ...
    const handleSubmit = async () => {
        ...
        if (response.status === HttpStatus.OK) {
            MessageService.success(
                client === null ? 'OAuth client created successfully' : 'OAuth client updated successfully',
            )
            setShow(false)
        } else {
            MessageService.error('Failed to create OAuth client')
        }
    }
                                ...
                                <Row>
                                    ...
                                    <Col>
                                        <Form.Control
                                            ...
                                            onChange={handleTokenUnitChange('access')}
                                        >
                                            <option>Days</option>
                                ...
                                <Row>
                                    ...
                                    <Col>
                                        <Form.Control
                                            ...
                                            onChange={handleTokenUnitChange('refresh')}
                                        >
                                            <option>Days</option>
                                            <option>Seconds</option>
                                        </Form.Control>
