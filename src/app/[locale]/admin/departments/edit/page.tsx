...
import SecondaryDeparments from '../components/SecondaryDepartments'
...
const EditDepartmentPage = (): JSX.Element => {
    ...
    const handleUpdateDepartmentMembers = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const response = await ApiUtils.PATCH(`secondaryDepartments/members?departmentName=${secondaryDepartmentName}`, memberEmails , session.user.access_token)
    ...
    const fetchDepartmentMembersEmails = useCallback(async () => {
        ...
        const response = await ApiUtils.GET(`secondaryDepartments/members`, session.user.access_token)
        if (response.status === HttpStatus.UNAUTHORIZED) {
            return signOut()
        }
        if (response.status !== HttpStatus.OK) {
        ...
        const departmentsWithEmails: SecondaryDeparments = await response.json()
                ...
                :
                    ...
                    >
                        <div>
                            ...
                            <Button variant='light' onClick={() => router.push('/admin/departments')}>
                                {t('Cancel')}
                            </Button>
                        </div>
                            ...
                            <tbody className='col-12'>
                                {
                                    Object.entries(memberEmails).map(([index, email]) => (
                                        <tr key={email} className='row'>
                                            ...
                                            <td className='col-6'>
                                                <FaTrashAlt size={20} className='btn-icon mt-2' onClick={() => deleteEmail(parseInt(index))}/>
                                            </td>
                                        </tr>
                                    ))
                                }
