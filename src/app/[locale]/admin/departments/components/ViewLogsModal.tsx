...
const ViewLogsModal = ({show, setShow}: Props): JSX.Element => {
    ...
    const fetchLogFileContentBySelectedDate = useCallback(async () => {
        ...
        const response = await ApiUtils.GET(`departments/logFileContent?date=${selectedDate}`, session.user.access_token)
        if (response.status !== HttpStatus.OK) {
            setLogFileContents([])
            return
        }
    ...
    return (
        <div>
            <Modal show={show} onHide={() => setShow(false)} backdrop='static' centered size='xl' dialogClassName='modal-info'>
                <Modal.Header closeButton>
                    <Modal.Title>{t('View Logs')} </Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <div>
                        <label htmlFor='file-log'><b>{t('Search')}</b></label>
                        <input list='file-logs' type='date' name='file-log'
                            id='file-log' defaultValue={selectedDate}
                    ...
                    <div className='row mt-3'>
                        <div className='col-12'>
                            <h5 className='header-underlined text-center'>{t('Log file on')}: {selectedDate}</h5>
                        </div>
                    </div>
                    <p className='row mt-3 log-content'>
                       {
                            Object.values(logFileContents).map((logFileContent, index) => (
                                 <span className='mb-3' key={index}>{logFileContent}</span>
                            ))
                       }
                    </p>
                </Modal.Body>
